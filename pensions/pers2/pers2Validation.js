...
export function validatePERS2Input(input = {}) {
    ...
    if (
        ...
        throw new Error("PERS2 requires a valid serviceYears number.");
    }
    ...
    if (
        ...
        throw new Error(
            `PERS2 retirementAge must be at least ${CONSTANTS.EARLY_RETIREMENT_MIN_AGE}.`
        );
    }
    ...
    if (
        ...
        throw new Error(
            "PERS2 requires a valid averageFinalCompensation number."
        );
    }

    if (serviceYears < CONSTANTS.VESTING_SERVICE_YEARS) {
        throw new Error(
            `PERS2 requires at least ${CONSTANTS.VESTING_SERVICE_YEARS} years of service to vest.`
        );
    }
    ...
    if (hireDate !== undefined && hireDate !== null && hireDate !== "") {
        ...
        if (Number.isNaN(normalizedHireDate.getTime())) {
            throw new Error(
                "PERS2 hireDate must be a valid date string when provided."
            );
        }
    }
}
