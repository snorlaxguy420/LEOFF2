...
- Account settings now let each user choose retirement check-in email frequency (`monthly`, `every 6 months`, `yearly`, or `never`), and the backend now includes a scheduled reminder job that only sends the check-in when that account is actually due in [ui/login.html](/D:/LEOFF%202/ui/login.html), [ui/login.js](/D:/LEOFF%202/ui/login.js), [backend/src/app.js](/D:/LEOFF%202/backend/src/app.js), [backend/src/sendRetirementCheckInEmails.js](/D:/LEOFF%202/backend/src/sendRetirementCheckInEmails.js), and [backend/src/lib/email.js](/D:/LEOFF%202/backend/src/lib/email.js)
